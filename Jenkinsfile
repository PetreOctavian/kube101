			...
			steps{
				script{
					...
					withKubeConfig([credentialsId: 'kubeconfig']) {
						...
						sh "kubectl set image deployments/db-deployment petreoctav/licenta:dbimageBN${env.BUILD_NUMBER} -n ${namespace}"
						sh "kubectl apply -f  web.yaml -n ${namespace}"
						sh "kubectl set image deployments/web-deployment petreoctav/licenta:webimageBN${env.BUILD_NUMBER} -n ${namespace}"
					}
					sh "sleep 60"
				}
			}
